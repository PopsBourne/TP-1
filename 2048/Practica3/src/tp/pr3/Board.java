...
public class Board {
	...
	*/
	...
	public void store(BufferedWriter out) throws IOException {
		for (int i = 0; i < _boardSize; i++) {
			for (int j = 0; j < _boardSize; j++) {
				out.write(_board[i][j].getValor() + "\t");
			}
			out.newLine();	
		}
	}
	public void load(BufferedReader out) throws IOException {
		String s  = out.readLine();
		String[] strings = s.split("\t");


		this._boardSize = strings.length;
		_board = new Cell [_boardSize][_boardSize];

		for(int j = 0; j < _boardSize; j++) {
			int k = Integer.parseInt(strings[j]);
			_board[0][j]= new Cell(k);
		}
		...
		for(int i = 1; i < _boardSize; i++) {
			...
			for(int j = 0; j < _boardSize; j++) {
				try {
				int k = Integer.parseInt(st[j]);
				_board[i][j]= new Cell(k);
				}
				catch(NumberFormatException e) {
					//System.out.println("Caracter invalido en la matriz");
